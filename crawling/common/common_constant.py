class commonConstant_NAME:
    # 하나의 collection으로 통합
    GANGNAM_NAME = "GANGNAM";                                       # 강남
    GANGNAM_BOROUGH_NOTICE = "GANGNAM_NOTICE";                       # 강남구
    GANGNAM_BOROUGH_NOTICE_EVENT = "GANGNAM_NOTICE_EVENT";                       # 강남구
    GANGNAM_BOROUGH_OTHER_INSTITUTIONS = "GANGNAM_INSTITUTIONS";     # 강남구

    GWANAK_NAME = "GWANAK";              # 관악
    GWANAK_BOROUGH_NOTICE = "GWANAK_NOTICE";
    # GWANAK_BOROUGH_OTHER_INSTITUTIONS = "GWANAK_INSTITUTIONS";

    GANGDONG_BOROUGH_NOTICE = "GANGDONG_NOTICE";                      # 강동구
    GANGDONG_BOROUGH_NOTICE_SECOND = "GANGDONG_NOTICE_SECOND";                      # 강동구
    GANGDONG_BOROUGH_OTHER_INSTITUTIONS = "GANGDONG_INSTITUTIONS";    # 강동구

    GANGBUK_NAME ="GANGBUK";             # 강북
    GANGBUK_BOROUGH_NOTICE = "GANGBUK_NOTICE";                      # 강북구
    GANGBUK_BOROUGH_OTHER_INSTITUTIONS = "GANGBUK_INSTITUTIONS";    # 강북구

    GWANGZIN_NAME = "GWANGZIN";          # 광진
    GWANGJIN_BOROUGH_NOTICE = "GWANGJIN_NOTICE";
    GWANGJIN_BOROUGH_OTHER_INSTITUTIONS = "GWANGJIN_INSTITUTIONS";

    GEUAMCHEOUN_NAME = "GEUAMCHEOUN";    # 금천
    GEUAMCHEOUN_BOROUGH_NOTICE = "GEUAMCHEOUN_NOTICE";
    GEUAMCHEOUN_BOROUGH_OTHER_INSTITUTIONS = "GEUAMCHEOUN_INSTITUTIONS";
    
    DOBONG_NAME = "DOBONG";              # 도봉
    DOBONG_BOROUGH_NOTICE = "DOBONG_NOTICE";
    DOBONG_BOROUGH_NOTICE_EVENT = "DOBONG_NOTICE_EVENT";
    DOBONG_BOROUGH_OTHER_INSTITUTIONS = "DOBONG_INSTITUTIONS";

    DONGDAEMUN_NAME = "DONGDAEMUN";      # 동대문
    DONGDAEMUN_BOROUGH_NOTICE = "DONGDAEMUN_NOTICE";
    DONGDAEMUN_BOROUGH_NOTICE_EVENT = "DONGDAEMUN_NOTICE_EVENT";
    DONGDAEMUN_BOROUGH_OTHER_INSTITUTIONS = "DONGDAEMUN_INSTITUTIONS";

    DONGJAK_NAME = "DONGJAK";            # 동작 
    DONGJAK_BOROUGH_NOTICE = "DONGJAK_NOTICE"; 
    DONGJAK_BOROUGH_NOTICE_EVENT = "DONGJAK_NOTICE_EVENT"; 
    DONGJAK_BOROUGH_OTHER_INSTITUTIONS = "DONGJAK_INSTITUTIONS";

    MAPO_NAME = "MAPO";           
    MAPO_BOROUGH_NOTICE = "MAPO_NOTICE";
    MAPO_BOROUGH_NOTICE_EVENT = "MAPO_NOTICE_EVENT";
    MAPO_BOROUGH_NOTICE_EDUCATION = "MAPO_NOTICE_EVENT_EDUCATION";
    MAPO_BOROUGH_OTHER_INSTITUTIONS ="MAPO_INSTITUTIONS";

    SEOCHO_NAME = "SEOCHO";              # 서초 
    SEOCHO_BOROUGH_NOTICE = "SEOCHO_NOTICE";
    SEOCHO_BOROUGH_NOTICE_EDUCATION = "SEOCHO_NOTICE_EDUCATION";
    SEOCHO_BOROUGH_NOTICE_EVENT = "SEOCHO_NOTICE_EVENT";

    SEONGDONG_NAME = "SEONGDONG";        # 성동
    SEONGDONG_BOROUGH_NOTICE = "SEONGDONG_NOTICE";

    SEONGBUK_NAME = "SEONGBUK";          # 성북
    # SEONGBUK_BOROUGH_NOTICE = "SEONGBUK_NOTICE";

    SONGPA_NAME = "SONGPA";              # 송파
    SONGPA_BOROUGH_NOTICE = "SONGPA_NOTICE";
    SONGPA_BOROUGH_NOTICE_EVENT = "SONGPA_NOTICE_EVENT";

    YANGCHEON_NAME = "YANGCHEON";        # 양천
    YANGCHEON_BOROUGH_NOTICE = "YANGCHEON_NOTICE";
    YANGCHEON_BOROUGH_NOTICE_EVENT = "YANGCHEON_NOTICE_EVENT";

    YEONGDEUNGPO_NAME = "YEONGDEUNGPO";  # 영등포 
    YEONGDEUNGPO_BOROUGH_NOTICE = "YEONGDEUNGPO_NOTICE";

    EUNPYENG_NAME = "EUNPYENG";          # 은평
    EUNPYENG_BOROUGH_NOTICE = "EUNPYENG_NOTICE";
    EUNPYENG_BOROUGH_NOTICE_EDUCATION = "EUNPYENG_NOTICE_EDUCATION";

    JONGRO_NAME = "JONGRO";              # 종로
    JONGRO_BOROUGH_NOTICE = "JONGRO_NOTICE";

    JUNGGU_NAME = "JUNGGU";              # 중구
    JUNGGU_BOROUGH_NOTICE = "JUNGGU_NOTICE";
    
    JUNGNANG_NAME = "JUNGNANG";          # 중랑
    JUNGNANG_BOROUGH_NOTICE = "JUNGNANG_NOTICE";

    GANGDONG_NAME = "GANGDONG";

    GANGSEO_NAME = "GANGSEO";           # 강서구
    GANGSEO_BOROUGH_NOTICE = "GANGSEO_NOTICE";                      

    GURO_NAME = "GURO";
    GURO_BOROUGH_NOTICE = "GURO_NOTICE";
    GURO_BOROUGH_OTHER_INSTITUTIONS = "GURO_INSTITUTIONS";

    NOWON_NAME = "NOWON";
    NOWON_BOROUGH_NOTICE = "NOWON_NOTICE"; 
    NOWON_BOROUGH_NOTICE_EVENT = "NOWON_NOTICE_EVENT"; 
    NOWON_BOROUGH_OTHER_INSTITUTIONS = "NOWON_INSTITUTIONS"; 

    YOUNGSAN_NAME = "YONGSAN";
    YOUNGSAN_BOROUGH_NOTICE = "YOUNGSAN_NOTICE";
    YOUNGSAN_BOROUGH_NOTICE_EVENT = "YOUNGSAN_NOTICE_EVENT";

    SEODAEMUN_NAME = "SEODAEMUN";

    # 서울시
    NPO_NAME = "NPO";                    # NPO
    SEOUL_NAME = "SEOUL";                # 서울
    YOUTHSEOUL_NAME = "YOUTHSEOUL"
    SBASEOUL_NAME ="SBASEOUL";
    JOBSEOUL_NAME = "JOBSEOUL";
    HOUSINGSEOUL_NANE ="HOUSINGSEOUL";
    #       서울시NPO지원센터	https://www.snpo.kr/bbs/board.php?bo_table=bbs_npo
    #       서울문화재단	https://www.sfac.or.kr/opensquare/notice/notice_list.do -> 링크 확인 필요
    #       청년몽땅정보통	https://youth.seoul.go.kr/site/main/board/notice/list?cp=1&pageSize=15&sortOrder=BA_REGDATE&sortDirection=DESC&bcId=notice&baCategory1=basic&baNotice=false&baCommSelec=true&baOpenDay=true&baUse=true
    #       SBA	https://www.sba.seoul.kr/Pages/CustomerCenter/Notice.aspx
    #       서울일자리포털	https://job.seoul.go.kr/Main.do?method=getMain -> 포털의 어느 부분인지
    #       내손안에 서울	https://mediahub.seoul.go.kr/main.do -> 포털의 어느 부분인지
    #       서울복지포털	https://wis.seoul.go.kr/ -> 포털의 어느 부분인지
    #       서울주거포털	https://housing.seoul.go.kr/site/main/board/notice/list
    #       50+	https://50plus.or.kr/notice_recruit.do

    STOPCUOUNT = 34;
    NOTICE_STOP_COUNT = 65;

    # 서울시 STOP COUNT
    SEOUL_STOP_COUNT_ONE = 32; # 서울시NPO지원센터
    SEOUL_STOP_COUNT_TWO = 62; # 청년몽땅정보통
    SEOUL_STOP_COUNT_THREE = 92;
    SEOUL_STOP_COUNT_FOUR = 122;
    SEOUL_STOP_COUNT_FIVE = 152;
    SEOUL_STOP_COUNT_SIX = 182;
    SEOUL_STOP_COUNT_SEVEN = 212;
    SEOUL_STOP_COUNT_EIGHT = 242;
    SEOUL_STOP_COUNT_NIGHT = 272;

    STATUS_SUCCESS_CODE = 200;